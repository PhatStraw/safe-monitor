...
async function saveSecondaryAccount(primary_id, user, data, youtubeData) {
  youtubeData.timestamp = new Date().toISOString();
  ...
  try {
    ...
    if (existingAccount.rows.length > 0) {
      ...
      const { rows } = await sql`
        ...
        VALUES (${primary_id}, ${email}, ${name}, ${access_token}, ${refresh_token}, ${JSON.stringify(youtubeData)})
        RETURNING *;
      `;
...
export async function POST(request) {
  try {
    ...
    if (summarizedData) {
      ...
      result = await saveSecondaryAccount(
        primary_id,
        secondaryInfo,
        data,
        summarizedData
    ...
    return NextResponse.json(
      { data: { name: secondaryInfo.name, secondaryId: account_id } },
      { status: 200 }
    );
  } catch (error) {
