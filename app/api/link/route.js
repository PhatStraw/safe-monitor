...
async function getUserData(auth) {
  ...
  for (let i = 0; i < data.subscriptions.length; i++) {
    ...
    const uploadsResponse = await service.search
      ...
      .catch((err) => console.error("Error retrieving last uploaded video for each subscription: " + err));
...
export async function POST(request) {
  try {
    ...
    // const youtubeData = await getUserData(data);
    const youtubeData = {
      "likedVideos": [
        {
    ...
    const summarizedData = await summarizeContent(youtubeData, email);
    console.log("summary", summarizeContent);
    await saveSecondaryAccount(user_id, userInfo, data, summarizedData);

    return NextResponse.json(
      { data: { ...data, ...userInfo, youtubeData: summarizedData } },
      { status: 200 }
    );
  } catch (error) {
